...
async def update_json(session, filename, start_user_id, end_user_id):
    ...
    urls = [f"https://fantasy.premierleague.com/api/entry/{user_id}/history/" for user_id in user_ids]
    ...
    for user, result in zip(data, results):
        user["chips"] = {"tc": None, "fh": None, "bb": None, "wc1": None, "wc2": None}
        if result == {"detail": "Not found."}:
            ...
            for chip in result["chips"]:
                if chip["name"] == "wildcard":
                    if chip["event"] <= 16:
                        user["chips"]["wc1"] = chip["event"]
                    elif chip["event"] > 16:
                        user["chips"]["wc2"] = chip["event"]
                elif chip["name"] == "3xc":
                    user["chips"]["tc"] = chip["event"]
                elif chip["name"] == "bboost":
                    user["chips"]["bb"] = chip["event"]
